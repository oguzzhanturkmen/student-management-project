package com.project.contactmessage.messages;

public class Messages {

    public static final String CONTACT_MESSAGE_NOT_FOUND = "Contact message not found";
    public static final String CONTACT_MESSAGE_DELETED = "Contact message deleted successfully";

    public static final String UNVALID_DATE_FORMAT = "Unvalid date format";
    public static final String UNVALID_TIME_FORMAT = "Unvalid time format";
}
